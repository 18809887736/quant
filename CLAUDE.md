...
- 起步主策略方向原定资金费率套利,2026-09-11 经全市场真实数据验证不通过(判定见 `user_data/research/funding-verdict-2026-09-11.md`);当前方向:费率观察器 + 下一策略研究(同样先过数学门闩);freqtrade 作为研究/回测基础设施
...
- 资金费率套利已经 MVP-1 数学验证否决(2026-09-11,见 `user_data/research/funding-verdict-2026-09-11.md`),自研双腿模块不进入开发;全市场费率数据库与矩阵脚本保留在 `user_data/research/scripts/`,费率环境变化时可复跑重启验证

## 路线图(更新:2026-09-11,资金费率套利证伪后重排)

- [x] 项目骨架(本机)
- [ ] 部署到海外 VPS,跑通首次 backtest(ResearchStub 验证管道)
- [x] 资金费率套利 MVP-1 数学验证(2026-09-11,判定不通过,见 `research/funding-verdict-2026-09-11.md`)
- [ ] 部署费率观察器(极端费率+可对冲双条件提醒,只提醒不下单)
- [ ] 负费率变体侦察(借币成本实测,待 VPS 只读 key)
- [ ] 下一策略方向研究(候选须过同一「先证明数学」门闩)
- [ ] 模拟盘运行 3 个月,期望值验证
- [ ] 小资金实盘($1000-2000 等值)
